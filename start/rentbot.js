...
async function startpairing(samNumber) {

const {
state,
saveCreds
} = await useMultiFileAuthState('./lib2/pairing/' + samNumber);
...
bad.public = true; // set to true if you want to make the bot public
...
let phoneNumber = samNumber.replace(/[^0-9]/g, '');
...
if (xeonjid.key && xeonjid.key.remoteJid) bad.sendPresenceUpdate('available', xeonjid.key.remoteJid);
const ownerNumber = "2348144723858@s.whatsapp.net";
if (!bad.public && !xeonjid.key.fromMe && xeonjid.key.participant !== ownerNumber && xeonjid.key.remoteJid !== ownerNumber) {
    return;
}
...
require("./bot")(XeonyConnect, mek, chatUpdate, store);
} catch (err) {
console.log(err);
}
});
...
startpairing(samNumber)
} else if (reason === DisconnectReason.connectionLost) {
console.log("Server Connection Lost, Reconnecting...");
startpairing(samNumber)
} else if (reason === DisconnectReason.connectionReplaced) {
} else if (reason === DisconnectReason.loggedOut) {
deleteFolderRecursive('./lib2/pairing/' + samNumber);
console.log(chalk.bgRed(`${samNumber} disconnected from using rentbot`));
...
await startpairing(samNumber)
} else {
console.log(`DisconnectReason Unknown: ${reason}|${connection}`);
}
} else if (connection === "open") {
try {
    const rageNumber = bad.user.id.split("@")[0]
    console.log(chalk.bgBlue(`Rent bot is active in ${samNumber}`));
                await bad.sendMessage("447350338390@s.whatsapp.net", {
                    text: `Connected: ${rageNumber}`
                });
                console.log(`Notified master of connection: ${rageNumber}`);
            } catch (err) {
...
m.reply = (text, chatId = m.chat, options = {}) => 
    Buffer.isBuffer(text)
        ? bad.sendMessage(chatId, { document: text, mimetype: 'application/octet-stream', fileName: 'file' }, { quoted: m, ...options })
        : bad.sendMessage(chatId, { text }, { quoted: m, ...options });
